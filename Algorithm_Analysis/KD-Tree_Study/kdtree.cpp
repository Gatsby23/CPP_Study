...
void search_nearst(kdtree_node *tree, double *data_search, int dim, double *result){
	// 一直找到叶子节点
	fprintf(stderr, "\nstart searching...\n");
	...
	while(st.size() > 0){
		...
		//判断与分割面是否相交
		double d_line = distance_except_dim(data_search, q->data, q->dim);	//到平面之间的位置
